...
/**
 * Class: OpenLayers.Format.ArcGisGetFeatureInfo
 * Class to read GetFeatureInfo responses from Web Mapping Services
 *
 * Inherits from:
 *  - <OpenLayers.Format.XML>
 */
OpenLayers.Format.ArcGisGetFeatureInfo = OpenLayers.Class(OpenLayers.Format.XML, {
    ...
    /**
     * Constructor: OpenLayers.Format.ArcGisGetFeatureInfo
    ...
    read_ArcGisJson: function (doc) {
        var response = [];
        var data = jQuery.parseJSON(doc);
        var results = data.results;
        var geometryFormat = new OpenLayers.Format.EsriGeoJSON();
        for (var i = 0, len = results.length; i < len; ++i) {
            var result = results[i];
            var attributes = {};

            var jlen = result.attributes.length;
            if (jlen > 0) {
                for (var j = 0; j < jlen; j++) {
                    var attribute = result.attributes.attributes[j];
            ...
            geomFormat.features.push({
                attributes: result.attributes,
                geometry : result.geometry
            });

            var features = geometryFormat.read(geomFormat);

            for (var k = 0; k < features.length; k++) {
                response.push(features[k]);
            }            
        }
        return response;
    },
    ...
    */
    ...
    CLASS_NAME: "OpenLayers.Format.ArcGisGetFeatureInfo"
