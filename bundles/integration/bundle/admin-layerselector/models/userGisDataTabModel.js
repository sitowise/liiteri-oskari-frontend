    ...
    define(['_bundle/collections/layerGroupCollection'], function (LayerGroupCollection) {
        return Backbone.Model.extend({
			...
			getClasses: function () {
				...
				var myPlacesGroup = {};
				myPlacesGroup.id = 1;
				myPlacesGroup.name = {"en": "My places", "fi": "Kohteet"};
				groups.userGisData.push(myPlacesGroup);
				
				var analysisGroup = {};
				analysisGroup.id = 2;
				analysisGroup.name = {"en": "Analysis", "fi": "Analyysit"};
				groups.userGisData.push(analysisGroup);
				
				var uploadedGroup = {};
				uploadedGroup.id = 3;
				uploadedGroup.name = {"en": "Uploaded objects", "fi": "Aineistot"};
				groups.userGisData.push(uploadedGroup);
				
				// cleanup old layer Groups since we always refresh from server for now
				me.layerGroups = [];
				me.loadGroups(groups, 'layerIds');
            },
            ...
            _mapLayersForGroup: function (group, groupingMethod) {
                ...
                _.each(this.layers.models, function(layer) {
					//debugger;
					//if (layer.getLayerType() == "MYPLACES" || layer.getLayerType() == "ANALYSIS" || layer.getLayerType() == "USERLAYER") {
					//	debugger;
					//	var asd = "asd";
					//}
					...
					if (layer.getLayerType() == type.toLowerCase()) {
						group.add(layer);
					}
                });
            },
