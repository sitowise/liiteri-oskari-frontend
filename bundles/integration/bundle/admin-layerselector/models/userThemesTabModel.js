    ...
    define(['_bundle/collections/layerGroupCollection'], function (LayerGroupCollection) {
        return Backbone.Model.extend({
            ...
            initialize: function () {
                ...
                this.filteredLayerGroups = this.attributes.grouping || [];
            ...
            applyFilter: function () {
                var i, j, layer, layerGroup, layerGroupItem;
                var filter = this.filter;
                var filteredGroups = [];

                if (filter == null || filter == "") {
                    for (i = 0; i < this.layerGroups.length; i++) {
                        layerGroup = this.layerGroups[i];
                        layerGroupItem = {
                            'id': layerGroup.id, 'layers': [], 'show': true, name: layerGroup.name
                        };
                        for (j = 0; j < layerGroup.models.length; j++) {                            
                            layer = layerGroup.models[j];
                            layerGroupItem.layers.push({ 'id': layer.getId(), 'show': true });
                        }
                        filteredGroups.push(layerGroupItem);
                    }
                }                    
                else {                    
                    for (i = 0; i < this.layerGroups.length; i++) {
                        layerGroup = this.layerGroups[i];
                        layerGroupItem = {
                            'id' : layerGroup.id, 'layers' : [], name: layerGroup.name
                        };
                        var isShownLayer = false;
                        for (j = 0; j < layerGroup.models.length; j++) {
                            layer = layerGroup.models[j];
                            if (layerGroup.matchesKeyword(layer.getId(), filter)) {
                                layerGroupItem.layers.push({ 'id': layer.getId(), 'show': true });
                                isShownLayer = true;
                            } else {
                                layerGroupItem.layers.push({ 'id': layer.getId(), 'show': false });
                            }
                        }
                        layerGroupItem.show = isShownLayer;
                        filteredGroups.push(layerGroupItem);
                    }                    
                }
                this.filteredLayerGroups = filteredGroups;
                this.set('filteredLayerGroups', this.filteredLayerGroups);
                this.trigger('change:filteredLayerGroups');
            },
            /**
             * return group titles
             *
            ...
            _parseObjectToGroup : function(item, groupingMethod) {
                ...
                if(groupingMethod) {
                    me._mapLayersForGroup(loadedGroup, item[groupingMethod]);
                }
                this.sortByName();
                return hasChanges;
            },
             ...
             * @private
             */
            _mapLayersForGroup: function (group, ids) {
                var me = this;
